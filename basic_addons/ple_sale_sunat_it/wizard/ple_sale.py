...
'icbp' as campo23,
CASE WHEN compra.otros is not Null THEN (compra.otros)::varchar ELSE '0.00' END as campo24,
CASE WHEN compra.total is not Null THEN (compra.total)::varchar ELSE '0.00' END as campo25,
CASE WHEN compra.divisa is not Null THEN
	CASE WHEN compra.divisa = 'PEN' THEN 'PEN' ELSE compra.divisa END
ELSE '' END as campo26,
CASE WHEN compra.tipodecambio is not Null THEN ( round(compra.tipodecambio,3) )::varchar ELSE '1.000' END as campo27,
CASE WHEN compra.fechadm is not Null THEN (to_char( compra.fechadm::date , 'DD/MM/YYYY'))::varchar ELSE '' END as campo28,
CASE WHEN compra.tipodocmod is not Null THEN compra.tipodocmod ELSE '' END as campo29,
CASE WHEN compra.seriemod is not Null THEN repeat('0',4-char_length(compra.seriemod)) || compra.seriemod ELSE '' END as campo30,
CASE WHEN compra.numeromod is not Null THEN compra.numeromod ELSE '' END as campo31,
CASE WHEN ai.numero_contrato_cliente is not Null THEN ai.numero_contrato_cliente else '' end as campo32,
CASE WHEN ai.inconsistencia_tipo_cambio_cliente THEN '1' else '' END as campo33,
CASE WHEN ai.cancelado_medio_pago_cliente THEN '1' ELSE '' END as campo34,
ai.estado_ple_venta as campo35,
'' as campo36
...
ELSE '' END as campo18,
CASE WHEN compra.tipodecambio is not Null THEN ( round(compra.tipodecambio,3) )::varchar ELSE '1.000' END as campo19,
CASE WHEN compra.fechadm is not Null THEN (to_char( compra.fechadm::date , 'DD/MM/YYYY'))::varchar ELSE '' END as campo20,
CASE WHEN compra.tipodocmod is not Null THEN compra.tipodocmod ELSE '' END as campo21,
CASE WHEN compra.seriemod is not Null THEN repeat('0',4-char_length(compra.seriemod)) || compra.seriemod ELSE '' END as campo22,
CASE WHEN compra.numeromod is not Null THEN compra.numeromod ELSE '' END as campo23,
CASE WHEN ai.inconsistencia_tipo_cambio_cliente THEN '1' else '' END as campo24,
CASE WHEN ai.cancelado_medio_pago_cliente THEN '1' ELSE '' END as campo25,
ai.estado_ple_venta as campo26,
'' as campo27
from get_venta_1_1_1(0,209501) compra
inner join account_period ap on ap.name = compra.periodo
inner join account_move am on am.id = compra.am_id
...
with delimiter '|'
		...
		if exp != '':
			...
			datos = exp.split('|')
			if self.tipo_ple == '141':
				for line in list(range(len(datos)/35)):
					start = line*35
					end = start + 35
					record = datos[start:end]
					move = self.env['account.invoice'].search([('reference', '=', record[7])], limit=1)
					if move.move_id.icbp != 0:
						record[22] = move.move_id.icbp
					else:
						record[22] = '0.00'
					estruct = ''
					for i in record:
			...
			elif self.tipo_ple == '142':
				for line in list(range(len(datos)/26)):
					start = line*26
					end = start + 26
					record = datos[start:end]
					move = self.env['account.invoice'].search([('reference', '=', record[7])], limit=1)
